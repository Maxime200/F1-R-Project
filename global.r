...
df_circuits_new <-df_circuits %>%
  mutate(df_circuits, continent = case_when(country=="UAE" ~ "Asia",
                                            country=="Japan" ~ "Asia",
                                            country=="China" ~ "Asia",
                                            country=="Korea" ~ "Asia",
                                            country=="India" ~ "Asia",
...
df_total <- df_constructor_results %>% group_by(constructorId) %>% summarise(pointsTotal = sum(points))

df_pit_stops_new <- df_pit_stops %>% 
  ...
  merge(df_total, by = "constructorId") %>%
...
Tracer_histo <- function(data_constructor, color) 
{
  histo <- ggplot() + 
    ...
    geom_histogram(data =  filter(df_pit_stops_new, constructorId == data_constructor), aes(x = seconds), binwidth = 0.5, color = "black", fill = color, alpha = 1) +
    # on définit le titre
